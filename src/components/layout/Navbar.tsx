    ...
    <>
      ...
      >
        <nav className="mx-auto flex w-full max-w-[100rem] items-center justify-between gap-4 px-4 sm:px-6 md:px-8 min-[1100px]:gap-3 min-[1100px]:px-10 xl:px-12">
          <Logo priority imageClassName="h-12 w-auto sm:h-14 min-[1100px]:h-16 xl:h-[4.75rem]" />
            ...
            <div className="flex flex-nowrap items-center">
              {navLinks.map((link) => (
                <Link
                  ...
                  href={link.href}
                  className={cn(
                    "relative shrink-0 whitespace-nowrap rounded-lg px-2 py-2 text-lg font-medium transition-colors min-[1100px]:px-2.5 xl:px-3",
                    pathname === link.href
                      ? "text-accent-deep"
              ...
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    href={link.href}
                    className={cn(
                      "block rounded-xl px-8 py-3.5 text-xl font-medium transition-colors",
                      pathname === link.href
